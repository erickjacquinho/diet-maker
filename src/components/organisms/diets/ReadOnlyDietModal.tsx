...
export function ReadOnlyDietModal({ isOpen, onClose, diet, patientName }: ReadOnlyDietModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-3xl max-h-dialog overflow-y-auto p-6">
        <DialogHeader className="flex flex-col gap-2 border-b border-border-divider pb-4">
          <div className="flex items-start justify-between gap-3">
            ...
            <Badge variant="neutral" className="flex items-center gap-1"><Lock aria-hidden="true" /> Somente leitura</Badge>
          </div>
        </DialogHeader>
                ...
                <article key={meal.id} className="flex flex-col gap-3 rounded-control border border-border-subtle bg-surface p-4">
                  ...
                  {meal.options.map((option) => (
                    <div key={option.id} className="flex flex-col gap-2 border-l-2 border-primary-soft pl-3">
                      ...
                      {option.items.map((item) => <div key={item.id} className="flex items-center justify-between gap-3 text-sm"><span>{item.name}</span><span className="text-text-secondary">{item.snapshot.prescribedQuantity}{item.snapshot.prescribedUnit} · {item.snapshot.prescribedNutrients.energyKcal ?? '—'} kcal</span></div>)}
                    </div>
                  ))}
                </article>
              ))}
        ...
        <DialogFooter><Button onClick={onClose} variant="secondary" size="compact">Fechar Visualização</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  );
}
