...
> **DIRETRIZ ABSOLUTA**: **ZERO VALORES HARDCODED / ZERO ABSOLUTOS DE COR OU ESPAÇAMENTO**. Todos os componentes listados abaixo utilizam EXCLUSIVAMENTE tokens semânticos e utilitários do Design System (`bg-warm-bg`, `bg-warm-card`, `bg-warm-inner`, `border-warm-border`, `border-warm-borderDark`, `text-warm-main`, `text-warm-secondary`, `text-warm-muted`, `bg-nutri-success-bg`, `text-nutri-success-text`, `bg-nutri-warning-bg`, `text-nutri-warning-text`, `font-display`, `font-body`, `font-mono`, `rounded-card`, `rounded-control`, `rounded-pill`).  
