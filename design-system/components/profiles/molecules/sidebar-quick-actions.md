...
Export and Restore remain visible in both densities. A missing callback sets native `disabled` and exposes an `aria-describedby` reason: `A ação Exportar backup ainda não está disponível nesta tela.` or `A ação Restaurar backup ainda não está disponível nesta tela.`
