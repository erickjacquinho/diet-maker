# NutriDiet Design System — Especificação completa
...
Este pacote define a linguagem visual, tokens, componentes, estados, implementação e governança do NutriDiet.
...
| --- | --- |
| `refs/UI/design-system-governance/` | Fundação visual e processo de evolução de componentes |
| `design-system/` | Especificações visuais e funcionais atualmente adotadas pelo projeto |
...
Este pacote é uma **referência de governança proposta**. Por estar em `refs/`, ele não substitui sozinho as fontes roteadas por `AGENTS.md`. Sua promoção para regra operacional deve ocorrer em alteração separada e explícita.
